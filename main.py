...
class upload:
	...
	def upload(self):
		if self.media in os.listdir(os.getcwd()):
			htm={'file':open(self.media,'rb')}
			if 'SUKSES' in requests.post(self.url, files = htm,data={'_upl':'Upload'}).text :
...
def link():
	...
	if ( pil == '1' or pil == '01' ):
		...
		subdo = 'http://immunotecorlandoflorida.com/'
		tipe()
	else:
